function [denoised_img] = function_denoise_img(noisy_img,use_bm3d)
%
...
disp(' ')
    ...
    % this is a subsampled green channel raw-data from a Fujifilm FinePix S9600 camera
    ...
    %ztilde=ztilde/(max(ztilde(:)));             %  or divide by max(ztilde(:)), if that can be taken as the clipping value
    ztilde=noisy_img;            %  or divide by max(ztilde(:)), if that can be taken as the clipping value
...
text_verbosity_main=0;     % print to screen nothing (0), or some information about the ongoing processing (1-3)   [default 3]
figure_verbosity_main=0;   % show figures?  0: no figures,  1: show some figures  [default 1]
...
figure_verbosity=0;              %  show/keep figures?        [default 1]
%                                     0: no figures
...
if use_bm3d==1;                      % use BM3D denoising algorithm  [default 1]
bm3d_profile='high';               % complexity profile of BM3D denoising algorithm  [default 'np']
%                                    'np'  normal profile
...
other_string='[Efztilde]= denoiseImage(fztilde, denoisingNetwork(''DnCNN''));' ;  % if use_bm3d==0
...
if figure_verbosity_main
    figure, (ztilde),title('ztilde   noisy raw observation'), colorbar
end
if check_errors
...
if figure_verbosity_main
    if check_errors
        ...
        linenn=230;denoised=uint16(y_hat);
        linenn=min(linenn,size(ztilde,1));
